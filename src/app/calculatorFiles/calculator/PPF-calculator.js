...
import { Container, OverlayTrigger, Tooltip as BootstrapTooltip } from "react-bootstrap";
...
const CalculatorOutput = ({ principalAmount, estimatedReturns, futureValue }) => (
  <div className="p-4 pb-3 pt-3 rounded-lg mt-[10px] sm:mt-[15px] md:mt-[25px] lg:mt-[-20px] h-inherit w-full lg:w-auto bg-[#E6F0FA] mx-auto">
    ...
    <div className="flex justify-between text-sm md:text-lg lg:text-[15px] font-extrabold bg-white text-black p-[10px] rounded-lg mt-3 ml-[-13px] mr-[-13px]">
      ...
      <span>₹{formatNumber(futureValue)}</span>
    </div>
  </div>
);
  ...
  return (
    <div>
        ...
        overlay={
          <BootstrapTooltip id="tooltip">
            <p className="text-white">
              This is Standard Value of Interest Rate
            </p>
          </BootstrapTooltip>
        }
      >
      ...
      <div className="relative flex items-center justify-between bg-white border border-gray-100 rounded-lg p-3 shadow-sm">
        ...
        isPercentage ? 
          <input
            ...
            onBlur={handleBlur}
            readOnly={true}
            className="w-full font-bold text-blue-600 text-[20px] 2xl:text-[25px] outline-none bg-transparent cursor-not-allowed"
        ...
        isYear ?
          ...
          /> :
            isPercentage ?
              ...
              value={value || 0}
              disabled={true}
              className="w-full mt-2 appearance-none h-2 rounded-full bg-gradient-to-r from-teal-400 to-blue-600 outline-none border cursor-not-allowed"
            />
            :
            <input
              type="range"
              min={500}
              max={timePeriodvalue * 150000}
              value={value || 0}
              onChange={(e) => onChange(parseInt(e.target.value))}
              className="w-full mt-2 appearance-none h-2 rounded-full bg-gradient-to-r from-teal-400 to-blue-600 outline-none border"
            />
      ...
      <div className="text-sm text-gray-500 flex justify-between mt-1">
        <span>
          {isYear ? "1 Yr" : isPercentage ? "1%" : `₹500`}
        </span>
        <span>
          {isYear ? "40 Yr" : isPercentage ? "100%" : `₹${formatIndianCurrency(timePeriodvalue * 150000)}`}
        </span>
      </div>
...
function Calculator() {
  ...
  const [expectedReturnRate, setExpectedReturnRate] = useState(7.1);
  ...
  const { totalInvestment, estimatedReturns, futureValue } = graphData;
  ...
  const postDataToAPI = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_CALCULATOR_API_URL}api/calculate/ppfCalculate`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        const { investedAmount, interestEarned, futureValue } = responseData;
          ...
          <div className="flex flex-col-reverse lg:flex-row justify-between lg:space-x-6 space-y-6 lg:space-y-0 text-[18px] 2xl:text-[22px] lg:gap-[50px]">
            ...
            <div className="responsive-input space-y-6 xl:space-y-9 2xl:space-y-6 w-full lg:w-[660px] xl:w-[900px] 2xl:w-[1050px] -ml-4 lg:-ml-5 lg:ml-2 lg:mx-0 mx-auto mt-[30px] lg:mt-0">
              <InputField
                ...
                min={500}
                max={timePeriod * 150000}
                timePeriodvalue={timePeriod}
                format={new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR' }).format(principalAmount)}
              />
              <InputField
                label="Interest Rate"
