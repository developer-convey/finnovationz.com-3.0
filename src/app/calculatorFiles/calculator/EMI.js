...
const CalculatorOutput = ({ principalAmount, estimatedReturns, totalAmount }) => (
  ...
  return (
    <div>
      ...
      <div className="text-sm text-gray-500 flex justify-between mt-1">
        <span>
          {isYear ? "1 Months" : isPercentage ? "1%" : `₹100,000`}
        </span>
        <span>
          {isYear ? "40 Months" : isPercentage ? "100%" : `₹10,000,000`}
        </span>
      </div>
...
function Calculator() {
  ...
  const { totalInvestment, estimatedReturns, totalAmount } = graphData;
  ...
  const postDataToAPI = async () => {
    try {
      ...
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_CALCULATOR_API_URL}api/calculate/emiCalculate`,
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        const { emi, totalInterst, totalAmount } = responseData;

        // Update the graph data state
        setGraphData({
          totalInvestment: parseFloat(emi),
          estimatedReturns: parseFloat(totalInterst),
