...
function CalculatorSip() {
  ...
  return (
    <div>
      {/* Render your calculator UI and use the API data as needed */}
...
const CalculatorOutput = ({ sipAmount, estimatedReturns }) => (
  <div className="p-4 pb-3 pt-3 rounded-lg mt-[10px] sm:mt-[15px] md:mt-[25px] lg:mt-[-20px] h-inherit w-full lg:w-auto bg-[#E6F0FA] mx-auto">
    ...
    <div className="flex justify-between text-sm md:text-lg lg:text-[15px] font-extrabold bg-white text-black p-[10px] rounded-lg mt-3 ml-[-13px] mr-[-13px]">
      ...
      <span>₹{formatNumber((sipAmount + estimatedReturns))}</span>
    </div>
  </div>
);
...
const InputField = ({
  ...
  const handleBlur = () => {
    if (value === "" || isNaN(value)) {
      onChange(0); // Reset to 0 if the field is left empty or invalid
    }
  };
  ...
  return (
    <div>
      ...
      <div className="text-sm text-gray-500 flex justify-between mt-1">
        <span>{isYear ? "1 Yr" : isPercentage ? "1%" : `₹500`}</span>
        <span>{isYear ? "40 Yr" : isPercentage ? "100%" : `₹1,000,000`}</span>
      </div>
...
function Calculator() {
  ...
  const calculateSIPReturns = () => {
    ...
    return { estimatedReturns, totalInvestment };
  };

  const generateGraphData = () => {
    const { totalInvestment, estimatedReturns } = calculateSIPReturns();
  ...
  const postDataToAPI = async () => {
    try {
      const data = {
        MonSIPAmount: sipAmount,
        years: timePeriod,
        rateOfInterest: expectedReturnRate,
      };
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_CALCULATOR_API_URL}api/calculate/sipReturn`,
        {
          method: "POST",
          headers: {
    ...
    <>
      <div className="p-4 mt-[-20px] calculator-container-position">
        ...
        >
          ...
          {/* Invest Now Button */}
          {/* <div className="responsive-button flex justify-center items-center">
            <button
              type="button"
              onClick={() => {
                postDataToAPI(); // Call the function here
              }}
              className="py-2.5 mt-[38px] bg-gradient-to-r from-teal-400 to-blue-600 text-white rounded-[40px] text-2xl font-medium shadow-md"
              style={{ width: "195px" }}
            >
              Submit
            </button>
