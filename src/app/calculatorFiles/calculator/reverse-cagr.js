...
const CalculatorOutput = ({ principalAmount, estimatedReturns, totalValue }) => (
  ...
  <div className="p-2 bg-white rounded-lg shadow-md text-gray-600 text-sm md:text-lg lg:text-[15px] font-medium">
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center">
        <span className="font-bold">Investment Required</span>
        <span className="text-transparent bg-clip-text bg-blue-600 font-extrabold">
...
function Calculator() {
  ...
  const postDataToAPI = async () => {
    try {
      const data = {
        expectedAmount: principalAmount,
        years: timePeriod,
        cagrRateOfReturn: expectedReturnRate,
      };

      const response = await fetch(
        `${process.env.NEXT_PUBLIC_CALCULATOR_API_URL}api/calculate/reCAGRCalculate`,
        {
          method: "POST",
          headers: {
